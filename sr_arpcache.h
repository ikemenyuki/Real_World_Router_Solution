/* This file defines an ARP cache, which is made of two structures: an ARP
   ...
   # The handle_arpreq() function is a function you should write, and it
     should handle sending ARP requests if necessary:
   ...
   # The ARP reply processing code should move entries from the ARP
     request queue to the ARP cache:
   ...
   ----------------------------------------------------------------------------
   FOR PROJECT 2a:
   Even though you are using the function arpcache_insert() here (since the
   return parameter gives you a pointer to the request queue that can be used
   to access all the packets waiting on this ARP reply), make sure you are not
   using the IP->MAC mapping anywhere, i.e. you should NOT use the
   arpcache_lookup() function anywhere in Project 2a. Instead, you must send
   an ARP request for every packet that the router is forwarding.
   ----------------------------------------------------------------------------
   FOR PROJECT 2b:
   Now you can make use of the arpcache_lookup() function. So before
   forwarding any packet, the router must lookup the ARP cache to check if it
   has a MAC entry corresponding to the next-hop IP. Thus, there is no longer
   a need to send an ARP request for every packet that is being forwarded.
   ----------------------------------------------------------------------------
